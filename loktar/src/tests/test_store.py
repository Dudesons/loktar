...
def test_store_test_env_on_s3(mocker):
    mocker.patch("loktar.store.s3")
    assert "s3:@:toto" == _store_test_env_on_s3("/tmp/toto", None, None)


@pytest.mark.parametrize("store_type", ["s3"])
def test_store_test_env(mocker, store_type):
    mocker.patch("loktar.store.s3")
    assert "{0}:@:toto".format(store_type) == store_test_env(store_type, "/tmp/toto")


@pytest.mark.parametrize("store_type", ["foobar"])
def test_store_test_env_fail_on_unknown_storage_method(mocker, store_type):
    mocker.patch("loktar.store.s3")
    with pytest.raises(UnknownStorageMethod):
        store_test_env(store_type, "/tmp/toto")


def test_get_back_test_env_on_s3(mocker):
    mocker.patch("loktar.store.s3")
    assert "/tmp/toto" == _get_back_test_env_from_s3("toto", None, None)
